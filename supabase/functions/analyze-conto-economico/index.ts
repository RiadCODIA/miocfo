...
serve(async (req) => {
  ...
  try {
    const { year, data } = await req.json();
    const LOVABLE_API_KEY = Deno.env.get("LOVABLE_API_KEY");
    if (!LOVABLE_API_KEY) throw new Error("LOVABLE_API_KEY is not configured");

    const prompt = `Sei un CFO virtuale esperto in PMI italiane. Analizza i seguenti dati del Conto Economico per l'anno ${year} e fornisci un report strutturato.
  ...
  "forecasts": "<previsione per i prossimi mesi in 2-3 frasi>"
    ...
    if (!response.ok) {
      if (response.status === 429) {
        return new Response(JSON.stringify({ error: "Troppo richieste. Riprova tra qualche minuto." }), {
          status: 429,
